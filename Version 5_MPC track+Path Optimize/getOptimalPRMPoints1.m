...
for i = 1 : PRMPathSize
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % xlim=[0 17.9] ylim=[0 17.2]
    
   if ((circleRadius.*cos(theta) + centre(1))<=17.9 ) & ((circleRadius.*sin(theta) + centre(2))<=17.2 )...
           & ((circleRadius.*cos(theta) + centre(1))>=0 ) & ((circleRadius.*sin(theta) + centre(2))>=0 )
